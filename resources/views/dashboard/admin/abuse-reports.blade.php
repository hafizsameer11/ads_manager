...
@section('title', 'Abuse Reports - Admin Dashboard')
...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <form method="GET" action="{{ route('dashboard.admin.abuse-reports') }}">
                <div class="filter-form-row">
                    ...
                    <div class="filter-field filter-field-search">
                        ...
                        <input type="text" name="search" class="form-control" placeholder="Search by email, description, or URL..." value="{{ request('search') }}">
                    </div>
                </div>
                <div class="filter-buttons">
                    <button type="submit" class="btn btn-primary">Filter</button>
                    <a href="{{ route('dashboard.admin.abuse-reports') }}" class="btn btn-secondary">Reset</a>
                </div>
            </form>
        </div>
    </div>
                    ...
                    <tbody>
                        @forelse($reports as $report)
                            <tr class="{{ $report->status == 'pending' ? 'unread' : '' }}">
                                <td>#{{ $report->id }}</td>
                                <td>
                                    <span class="badge badge-{{ $report->type }}">{{ ucfirst($report->type) }}</span>
                                </td>
                                <td>{{ $report->email }}</td>
                                <td>
                                    @if($report->url)
                                        <a href="{{ $report->url }}" target="_blank" rel="noopener noreferrer" style="color: #3498db; text-decoration: none;">
                                            {{ \Illuminate\Support\Str::limit($report->url, 30) }}
                                        </a>
                                    @else
                                ...
                                <td>
                                    <div class="message-preview" title="{{ $report->description }}">
                                ...
                                <td>
                                    <div class="action-buttons">
                                        <a href="{{ route('dashboard.admin.abuse-reports.show', $report->id) }}" class="btn btn-sm btn-primary" title="View">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <form action="{{ route('dashboard.admin.abuse-reports.destroy', $report->id) }}" method="POST" class="action-form" onsubmit="return confirm('Are you sure you want to delete this report?');">
                                            @csrf
                                            @method('DELETE')
