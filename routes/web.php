...
Route::get('/fix-advertiser-view', function () {
    $oldPath = resource_path('views/dashboard/Advertiser');
    $tempPath = resource_path('views/dashboard/advertiser_temp');
    $newPath = resource_path('views/dashboard/advertiser');
    ...
    if (is_dir($oldPath) && !is_dir($newPath)) {
        ...
        if (rename($oldPath, $tempPath)) {
            $messages[] = "Renamed Advertiser to advertiser_temp";
            if (rename($tempPath, $newPath)) {
                $messages[] = "Renamed advertiser_temp to advertiser";
            } else {
                $messages[] = "ERROR: Failed to rename advertiser_temp to advertiser";
                // Try to restore
                rename($tempPath, $oldPath);
            }
        } else {
            $messages[] = "ERROR: Failed to rename Advertiser directory";
        }
    } elseif (is_dir($newPath)) {
        $messages[] = "Directory 'advertiser' already exists (correct name)";
    } else {
        $messages[] = "Neither Advertiser nor advertiser directory found";
    }
    ...
    try {
        ...
        $messages[] = "View cache clear failed: " . $e->getMessage();
    }
    ...
    try {
        ...
        $messages[] = "Config cache clear failed: " . $e->getMessage();
    }
    ...
    try {
        ...
        $messages[] = "Route cache clear failed: " . $e->getMessage();
    }
    
    try {
        Artisan::call('optimize:clear');
        $messages[] = "Optimize cache cleared";
    } catch (\Exception $e) {
        $messages[] = "Optimize cache clear failed: " . $e->getMessage();
    }
    
    return response()->json([
        'success' => true,
        'messages' => $messages,
        'note' => 'Please remove or secure this route after fixing the issue!'
    ], 200);
})->name('fix.advertiser.view');
